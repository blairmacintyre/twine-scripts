...
(function () {
  ...
  // UTM conversion library from https://github.com/urbanetic/utm-converter
  ...
  */
  ...
  function coordToArray(coord, xProp, yProp) {
    ...
    // 
    // things related to Sugarcube macros
    //
    
    window.distanceToLocation = function(name) {
        ...
        var locUTM = locations[name].utm;
        
        if (locUTM.zone != location.utm.zone) {
            return -locUTM.zone;
//            return -2;
        }
        
        var dx = locUTM.coord.x - location.x;
        var dy = locUTM.coord.y - location.y;
        return Math.sqrt(dx*dx + dy*dy);
    }
    
    window.getLocation = function(name) {
    	var locations = Macro.get('newgeolocation').locations;
		...
		Macro.add('glstart', {
			handler : function () {
				...
				function onSuccess(position) {
                    ...
                    svl.utm =  converter.toUtm({coord: [glc.longitude,glc.latitude]});
                  
                    svl.x = svl.utm.coord.x;
		...
		*/
		...
		Macro.add('newgeolocation', {
			handler() {
				...
				var
					locations   = this.self.locations,
					id      = this.args[0],
                    lat     = parseFloat(this.args[1]),
                    lon     = parseFloat(this.args[2]);
